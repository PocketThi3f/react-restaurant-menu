...
class Inventory extends React.Component {
	...
	handleChange(e, key) {
		...
		// take a copy of that fish coming from new data
		const updatedFish = {
			...fish,
	...
	authHandler(err, authData) {
		...
		storeRef.once("value", (snapshot) => {
			...
			// claim it as our own if there is no owner at the moment
			if(!data.owner) {
				storeRef.set({
		...
		return (
			<div className="fish-edit" key={key}>
				...
				<textarea type="text" name="desc" value={fish.desc}
				 placeholder="Fish Desc" onChange={(e) => this.handleChange(e, key)}></textarea>
	...
	render() {
		...
		// check if they are the actual owner of the store
		if(this.state.uid !== this.state.owner) {
			return (
				<div>
					<p>Sorry, you can't get into a store you don't own!</p>
					{logout}
				</div>
			)
		}
